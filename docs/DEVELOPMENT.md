...
`Game::arrivalAge` 和 `shoreIdle` 仅为本局显示状态，不写入存档、不消耗 RNG。v0.4.3 开场由约 1.1 秒椅架撑开、0.45 秒停留、0.4 秒短淡出/淡入及落稳组成，总长仍 2.6 秒，输入立即跳过，继续处理原动作。`quietShore` 直接在既有 RGB565 帧缓冲中绘制，不加载概念图或视频，没有新增屏幕缓冲；窗灯常亮，倒影和水纹缓慢变化（转场时随整幅画面淡出/淡入）。问候在 2.6–7 秒显示并淡出，提示在 7–12 秒出现；之后只留水岸。操作后提示显示 4 秒。

`dist/arrival.gif` 是同一状态机和绘图函数产生的 15 秒放大预览，`rest-240x135.png` 为原分辨率。ASan/UBSan 覆盖三个场景、四种收藏保存状态和动画阶段。设备帧率、按键实际手感仍需真机验收。

椅架横向跨度增加时，竖向高度由固定杆长计算，双脚共享地面高度。站立的整椅和坐姿扶手分别绘制，短转场遮住视角切换，避免将靠背拉伸成扶手。杯子横坐标固定。
