...
name = 'PocketFishing-ADV-v0.2.0.bin'
...
assert all(marker in tests for marker in ['renderer:', 'journal:', 'compatibility:', 'objects: 24', 'journal CLI: C++ mixed']), 'native/CLI tests did not finish'
flash = re.search(r'Flash:.*used (\d+) bytes', buildlog)
ram = re.search(r'RAM:.*used (\d+) bytes', buildlog)
manifest = {'name':'PocketFishing','version':'0.2.0','theme':'phosphor terminal / Listening Tide dossiers (no avatar)','target':'M5Stack Cardputer ADV / ESP32-S3',
    ...
    'fish_family_dossiers':8, 'dossier_pages':3, 'follow_assist':True,
...
(dist/'安装说明.txt').write_text('''口袋钓鱼 v0.2.0｜听潮档案｜Cardputer ADV

把 PocketFishing-ADV-v0.2.0.bin 复制到已有 FAT32 SD 卡，例如 /Games/。
...
每次抛竿的物品概率仍约九分之一；自动跟鱼、空格收放线沿用上一版。

升级前请在电脑备份 /PocketFishing/catches-v1.pfj（若已有存档）。
新版本直接读取旧记录，不迁移或清空；新渔获使用生成器版本 2，仍每条 32 字节。
旧版固件不认识版本 2，回退后会在首条新记录处只读，不能显示后面的渔获。
...
screens.png / specimens.png / objects.png / lore.png / gameplay.gif 是同一绘图代码的电脑预览，不是实拍。
先验证按键、颜色、中文、成功保存和重启保留，再反馈钓鱼难度。
''')
entries=[name,'manifest.json','SHA256SUMS.txt','安装说明.txt','screens.png','specimens.png','objects.png','lore.png','gameplay.gif']
archive=dist/'PocketFishing-ADV-v0.2.0.zip'
